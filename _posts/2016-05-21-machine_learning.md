...
The expected information is **entropy**: $H(X) = E(I(X)) = \sum_{x} p_x I(x)  = -\sum_x p_x log_2{p_x}$
...
Entropy = $-6 \times (\frac{1}{6} \times \log_2 \frac{1}{6}) \approx 2.58$ so after rolling a die, I gain more information than flipping a coin. It's more unpredictable.
...
- linear regression $J(\theta) = \frac{1}{2n} \sum\limits_{i=1}^{n}(h_{\theta}(x^{(i)}) - y^{(i)})^2 + \frac{\lambda}{2} \theta^2 $
 - Support Vector Machines (SVM) $J(\theta) = \frac{1}{n} \sum\limits_{i=0}^{n} \max(1-y_i(\theta.x_i),0) + \frac{\lambda}{2} \theta ^2$
with $\theta$ vector of weights, $x_i$ vector of instance data, and $\frac{\lambda}{2} \theta^2$ the regularization term
